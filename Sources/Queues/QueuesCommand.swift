...
public final class QueuesCommand: Command {
    ...
    private func schedule(_ job: AnyScheduledJob, previousDate: Date? = nil) {
        if self.isShuttingDown.load() {
        ...
        if let task = job.schedule(context: context, previousDate: previousDate) {
            ...
            task.done.whenComplete { result in
                ...
                self.schedule(job, previousDate: task.scheduledDate)
            }
        }
    }
